 ...
 */
@WebServlet(description = "新增管理员用户", urlPatterns = { "/AddUserServlet_old" })
public class AddUserServlet_old extends HttpServlet {
		...
		if(sb.length() > 0) {
			...
			if (userService.isExistsUserName(userName)) {
				...
				if (userService.register(user)) {
					...
					response.sendRedirect(request.getContextPath() + "/login.jsp?login_message=" + mes);
